...
#	
#------------------------------------------------------------------
def slowLambert(X_0, X_f, dt_0, **kwargs):
...
####################################################################
	...
	nu_0 = arctan2(r_0[1],r_0[0])
	nu_f = arctan2(r_f[1],r_f[0])
	delta_nu = nu_f - nu_0
	...
	cos_delta_nu = r_0.dot(r_f)/(norm(r_0)*norm(r_f))
	A = DM * sqrt(norm(r_0)*norm(r_f)*(1+cos_delta_nu))
	...
	while abs(dt - dt_0) > 1e-6 and i<200:
		...
		y = norm(r_0) + norm(r_f) + A*(psi*c3 - 1)/sqrt(c2)
